...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2)    // If there aren't command-line arguments,
    mem_size = 50; // use 50 as the defualt value.
  else
    mem_size = atoi(argv[1]);

  printf("\t[+] allocating %d bytes of memory on the the heap for char_ptr\n", mem_size);
  ...
  *int_ptr = 31337; // Put the value of 31337 where int_ptr is pointing.
  ...
  if (char_ptr == NULL) { // Error cheacking, in case malloc() fails
    fprintf(stderr, "Error: counld not allocate heap memory.\n");
    exit(-1);
  }
